...
(function () {
  ...
  const CONFIG = {
    ...
    BANNER_MATCHERS: [
      {
        ...
        // Word-form counts ("Sechs bis zehn"). NOTE: deliberately no "einer" —
        // it appears inside the banner phrase ("aufgrund einer Beschwerde")
        // and must not be rewritten.
        numberWords: {
          eine: 1, zwei: 2, drei: 3, vier: 4, 'fünf': 5, sechs: 6, sieben: 7,
          acht: 8, neun: 9, zehn: 10, elf: 11, 'zwölf': 12, zwanzig: 20,
          einundzwanzig: 21, 'fünfzig': 50, 'einundfünfzig': 51, hundert: 100,
      ...
      },
      {
        ...
        // Word-form counts ("Six to ten"). Longer phrases must come first at
        // application time (handled by the parser sorting by key length).
        numberWords: {
          one: 1, two: 2, three: 3, four: 4, five: 5, six: 6, seven: 7,
          eight: 8, nine: 9, ten: 10, eleven: 11, twelve: 12, twenty: 20,
          'twenty-one': 21, fifty: 50, 'fifty-one': 51, 'one hundred': 100,
          hundred: 100, 'two hundred and fifty': 250, 'two hundred fifty': 250,
        },
      },
    ],
    ...
    /**
     * Header line that renders in the same block as the disclosure banner
     * ("Reviews aren't verified" / "Rezensionen werden nicht überprüft").
     * Its presence means the banner region has rendered, so a clean verdict
     * is safe to reach quickly. Keywords for the XPath first pass follow.
     */
    REVIEWS_META_TEXT: /Reviews\s+aren[’']t\s+verified|Rezensionen\s+werden\s+nicht\s+überprüft/i,
    REVIEWS_META_KEYWORDS: ['verified', 'überprüft'],
