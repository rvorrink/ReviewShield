...
(function () {
   ...
   *
   * Handles: "1", "2 bis 5", "über 250" (de) and "1", "2 to 5", "over 250" (en).
   * For the open-ended top bucket ("über/over N") the result is
   * [N + 1, worstCaseMax] with `openEnded: true` and `base: N`.
   *
  ...
  function parseRemovalRange(text, worstCaseMax, matchers) {
    ...
    for (const m of list) {
      ...
      const context = norm.slice(start, bannerMatch.index + bannerMatch[0].length);
      ...
      // Single exact count, e.g. "1 Bewertung … entfernt". Take the number
      // closest to the banner phrase within the context window.
      const before = context.slice(0, bannerMatch.index - start);
  ...
  /**
   * Corrected average assuming `removed` reviews were all 1-star:
   * (rating * count + 1 * removed) / (count + removed), clamped to [1, 5].
   * Returns null on invalid input.
   */
  function correctedRating(rating, count, removed) {
    ...
    const value = (rating * count + removed) / (count + removed);
    return Math.min(5, Math.max(1, value));
  }

  /**
   * Corrected rating bounds for a removal range.
   * More removed 1-star reviews → lower average, so:
   *   low  = corrected with range.max, high = corrected with range.min.
   * Returns {low, high} (unrounded) or null.
   */
  function correctedRange(rating, count, range) {
    if (!range) return null;
    const low = correctedRating(rating, count, range.max);
    const high = correctedRating(rating, count, range.min);
